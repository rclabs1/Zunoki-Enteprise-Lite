...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Starting Google Search Console OAuth flow for user:', userId);

    // Get environment variables (reuse Google Ads credentials for Google Search Console)
    ...
    oauthUrl.searchParams.set('scope', 'https://www.googleapis.com/auth/webmasters.readonly');
    ...
    console.log('Redirecting to Google Search Console OAuth:', oauthUrl.toString());

    // Redirect to Google OAuth
    return NextResponse.redirect(oauthUrl.toString());

  } catch (error) {
    console.error('Google Search Console OAuth initiation error:', error);
