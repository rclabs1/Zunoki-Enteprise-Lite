...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Processing YouTube Analytics OAuth callback for user:', userId);
    ...
    console.log('📺 YouTube Analytics: Fetching Google account information...');
    ...
    // Get YouTube channel info and analytics data
    console.log('📺 YouTube Analytics: Fetching YouTube channels...');
    let accountInfo = { totalChannels: 0, channels: [] };
    ...
    try {
      ...
      if (channelsResponse.ok) {
        ...
        console.warn('⚠️ YouTube channels fetch failed, but continuing with connection');
      }
    } catch (apiError) {
      console.warn('⚠️ YouTube API error:', apiError);
      // Continue with connection even if channels fetch fails
    }
    ...
    try {
      ...
      const { error } = await supabaseClient
        ...
        .upsert({
          ...
          token_data: {
            ...
            connection_metadata: {
              data_count: accountInfo.totalChannels || 0,
              status: (accountInfo.totalChannels || 0) > 0 ? 'active' : 'no_access',
      ...
      console.log('OAuth tokens stored successfully for user:', userIdentity.firebaseUid, 'platform:', 'youtube');
    } catch (storeError) {
    ...
    try {
      await supabaseMultiUserService.trackActivity('oauth_connected', {
        ...
        accounts: accountInfo.totalChannels || 0,
        user_id: userIdentity.firebaseUid // Pass Firebase UID in details since getCurrentUser() doesn't work
      }, 'youtube_analytics');
      console.log('✅ YouTube Analytics: Activity tracked successfully');
    } catch (activityError) {
      console.error('⚠️ YouTube Analytics: Failed to track activity:', {
        error: activityError,
    ...
    if (isSetupFlow) {
      ...
      return NextResponse.redirect(new URL('/onboarding/setup?success=youtube_analytics_connected', process.env.NEXT_PUBLIC_APP_URL!));
    } else {
      // Redirect back to Ad Hub with success (existing behavior)
      return NextResponse.redirect(new URL('/shell?module=ad-hub&success=youtube_analytics_connected', process.env.NEXT_PUBLIC_APP_URL!));
    }

  } catch (error) {
    console.error('YouTube Analytics OAuth callback error:', {
      error,
