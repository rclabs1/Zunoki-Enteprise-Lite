  ...
  try {
    ...
    const tokenResponse = await fetch('https://graph.facebook.com/v18.0/oauth/access_token', {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/callback/meta-ads`,
      }),
    });
    ...
    // Get account info
    const accountResponse = await fetch(`https://graph.facebook.com/v18.0/me/adaccounts?access_token=${tokens.access_token}`);
    const accountInfo = await accountResponse.json();

    // Store tokens with Firebase UID
    await supabaseMultiUserService.storeTokens(userIdentity.firebaseUid, 'meta_ads', {
    ...
    await supabaseMultiUserService.trackActivity('oauth_connected', {
      ...
      accounts: accountInfo.data?.length || 0,
    });

    return NextResponse.redirect(new URL('/dashboard?success=meta_ads_connected', request.url));
