...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Starting Google My Business OAuth flow for user:', userId);

    // Get environment variables (reuse Google Ads credentials for Google My Business)
    const clientId = process.env.GOOGLE_ADS_CLIENT_ID;
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/callback/google-my-business`;
    ...
    console.log('Redirecting to Google My Business OAuth:', oauthUrl.toString());

    // Redirect to Google OAuth
    return NextResponse.redirect(oauthUrl.toString());

  } catch (error) {
    console.error('Google My Business OAuth initiation error:', error);
