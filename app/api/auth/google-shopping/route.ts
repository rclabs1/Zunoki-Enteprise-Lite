...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Starting Google Shopping OAuth flow for user:', userId);

    // Get environment variables (reuse Google Ads credentials for Google Shopping)
    const clientId = process.env.GOOGLE_ADS_CLIENT_ID;
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/callback/google-shopping`;
    ...
    oauthUrl.searchParams.set('scope', 'https://www.googleapis.com/auth/content');
    ...
    console.log('Redirecting to Google Shopping OAuth:', oauthUrl.toString());

    // Redirect to Google OAuth
    return NextResponse.redirect(oauthUrl.toString());

  } catch (error) {
    console.error('Google Shopping OAuth initiation error:', error);
